    ...
    try {
      const response = await fetch('/api/chatbot-submit', {
        ...
        body: JSON.stringify({
          ...
          telephone: '',
          email: '',
          source: 'chatbot_retraite'
        })
      });
      if (!response.ok) throw new Error('Erreur lors de la sauvegarde');
      const result = await response.json();
      console.log('RetraiteChatBot data saved:', result);
      return result;
    } catch (err) {
...
export default RetraiteChatBot; 